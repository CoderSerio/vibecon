...
const props = defineProps<{
  ...
  savedAnnotation: (
    entry: LogEntry,
  ) => { previous_report?: unknown; label: Label } | undefined;
...
const storedSensitivity = Number(
  localStorage.getItem("vibecon.motion.pointerSensitivity"),
);
const motionSensitivity = ref(
  Number.isFinite(storedSensitivity) &&
    storedSensitivity >= 1 &&
    storedSensitivity <= 30
    ? storedSensitivity
    : 8,
);
  ...
  <section class="visualizer panel three-debug-panel">
      ...
      <div class="three-header-tools">
        ...
        <div class="three-debug-actions">
          <div class="three-view-switcher" :aria-label="t('debug.threeView')">
            <button
              v-for="view in ['front', 'rail', 'shoulder'] as const"
              :key="view"
              type="button"
              :class="{ active: inspectionView === view }"
              @click="inspectionView = view"
            >
              {{ t(`debug.threeView_${view}`) }}
            </button>
          </div>
          ...
          <label
            v-if="!hasFusionOrientation"
            class="motion-sensitivity compact"
          >
            <span>{{ t("debug.threeSensitivity") }}</span>
            <input
              v-model.number="motionSensitivity"
              type="range"
              min="1"
              max="30"
              step="1"
          ...
          <button class="secondary" type="button" @click="resetThreePose">
            {{ t("debug.threeReset") }}
          </button>
        </div>
      </div>
    ...
    <div class="three-joycon-stage">
      ...
      <div class="axis-readout three-axis-readout">
        ...
        ><output class="axis-output imu-output">{{
          renderImu(leftImu)
        }}</output>
  ...
  <section v-if="showLogs" class="panel transcript-panel">
    <div class="section-heading log-heading">
      ...
      <div class="log-actions">
        <span class="log-count" title="visible / retained"
          >{{ visibleLogEntries }}/{{ totalLogEntries }}</span
        >
        <label
          >{{ t("debug.logPolicy") }}
          <select
        ...
        <button class="secondary" @click="emit('clear')">
          {{ t("debug.clear") }}
        </button>
      </div>
    </div>
    <div class="transcript">
      <template v-if="visibleLogGroups.length">
        <div
          v-for="group in visibleLogGroups"

...
## 动作采样 Playground 与外部研究

在继续做动作映射之前，我们先把连续 IMU 采样设计成一个独立的 Playground。它的状态机、数据格式、质量门禁，以及对 Joy-Con 协议、姿态融合和惯性手势研究的摘要，单独记录在 [`imu-capture-playground.zh-CN.md`](./imu-capture-playground.zh-CN.md)。

这里有三个值得记住的研究结论：

1. Joy-Con 的传感器轮询频率高于控制器状态报告频率，UI 节流不能代替原始采样。
2. 姿态融合应该用四元数积分陀螺仪，并只在加速度模长接近 1g 时使用重力反馈；挥动时不能无条件信任加速度计。
3. 5 次重复适合做 MVP 验证，不足以训练通用模型；训练/验证必须按参与者划分，避免同一人的动作泄漏到两边。

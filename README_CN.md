...
- **实验性鼠标控制：** 摇杆模式用摇杆移动光标；体感模式把可配置的 `30°–120°` Joy-Con 旋转映射到当前显示器，并根据转动速度平滑调节增益。L/R 负责左键与拖拽，ZL/ZR 负责右键或体感 clutch，SL/SR 调节体感精度，短按 −/+ 复位，长按 −/+ 切换模式。独立测试会回读实际光标坐标，不会只凭事件发送调用成功就宣称鼠标可用。
